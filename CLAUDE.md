...
`src/aiController.js` — `computeAIDropSlot` (random drop-slot selection) plus `AI_THINK_DELAY` / `AI_MOVE_DELAY` timing constants. `src/aiControllerAdvanced.js` — `chooseAIBlockAdvanced`, `aiPersonality` (cautious/balanced/aggressive), and `minimaxAI`; difficulty adapts using `getRecentHistory()` from the score tracker. All AI turn logic lives in `src/hooks/useAIPlayer.js`.
